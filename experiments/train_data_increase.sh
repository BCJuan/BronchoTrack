...
function one_patient_one_traj () {
    python organize.py --root /mnt/DADES/datasetcalibracio/ --new-root data/cleaned --clean --n-trajectories $2 --only-val --test-patient "$1" --length 2

    python train.py --root data/cleaned/ --image-root /mnt/DADES/datasetcalibracio/ \
    --ckpt-name bronchonet_"$2"traj_COS_EU_inter_"$1" --batch-size 16 --modelo doublelate  \
    --gpus 1,2 --accelerator 'ddp' --lr 0.0001 --rot-loss cos

    python train.py --root data/cleaned/ --image-root /mnt/DADES/datasetcalibracio/ \
    --ckpt-name bronchonet_"$traj"traj_COS_EU_inter_"$1"  --batch-size 1 --modelo doublelate  --gpus=1 \
...
function intra_one_traj () {
    ...
    python train.py --root data/cleaned/ --image-root /mnt/DADES/datasetcalibracio/ \
    --ckpt-name bronchonet_"$1"traj_COS_EU_intra --batch-size 16 --modelo doublelate  \
    --gpus 1,2 --accelerator 'ddp' --lr 0.0001 --rot-loss cos

    python train.py --root data/cleaned/ --image-root /mnt/DADES/datasetcalibracio/ \
    --ckpt-name bronchonet_"$1"traj_COS_EU_intra  --batch-size 1 --modelo doublelate  --gpus=1 \
